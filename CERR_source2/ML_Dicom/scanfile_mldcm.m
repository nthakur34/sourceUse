...
%Close input stream.
in.close
tag1 = org.dcm4che3.data.Tag.Modality;
tag2 = org.dcm4che3.data.Tag.DirectoryRecordSequence;
%modalityTag = '00080060';
%currentModality = attrData.getString(hex2dec(modalityTag));
%disp(currentModality);
%modalityTag = '00080060';
%modality = dcm2ml_Element(attrData.get(modalityTag));

if (attrData.contains(tag1) && ~attrData.contains(tag2))
    disp('TRUEEEE');
else
    disp('WRONGGG');
end
