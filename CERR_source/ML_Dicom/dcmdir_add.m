...
%}
%%
patienttemplate = build_module_template('patient'); %<<<---changed buildmodule
patient = filter(dcmobj, patienttemplate);
...
study = filter(dcmobj, studytemplate);
...
for i=1:length(patientS.STUDY)
    ...
    %thisUID = patientS.STUDY(i).info.filter(attr);
    thisUID = filter(patientS.STUDY(i).info, attr);
...
series = filter(dcmobj, seriestemplate);
...
for i=1:length(studyS.SERIES)
    ...
    %thisUID = studyS.SERIES(i).info.filter(attr);
    thisUID = filter(studyS.SERIES(i).info, attr);
...
%% function for subset
function patient = filter(dcmobj, patienttemplate)
...
for i=1:length(tags)
    ...
    if patienttemplate.contains(tag)
        patient.setValue(tag, dcmobj.getVR(tag), dcmobj.getValue(tag));
    end 
end
